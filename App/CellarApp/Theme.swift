...
// MARK: - 主题 token（规格 §2.5：Phase 3 风格系统 v1 定版）

/// Cellar 面板色彩 token。**App/CellarApp 内 Color 字面量仅允许出现在本文件**
/// （执行门 G2：组件一律经 environment 读取 token；默认 .native = 系统语义色，
/// 原生极简基线，深浅色自动合规）。
///
...
struct CellarTheme {
    ...
    var accentGlow: AccentGlow
    /// 已解析语汇表（组合根构建 theme 时经 String(localized:) 一次性解析；
    /// 组件经 word(_:) 读取）。
    var vocabulary: [VocabularyWord: String]
    ...
    static let amberLight = CellarTheme(
    ...
    static let amberDark = CellarTheme(
    ...
    static func resolve(style: PanelStyle, scheme: ColorScheme) -> CellarTheme {
        switch style {
        case .native:
    ...
    static func swatchColors(style: PanelStyle, scheme: ColorScheme) -> [Color] {
        switch style {
        case .native:
            return [
    ...
    /// 语汇表构建：String(localized:) 逐词条解析（缺翻译的完整性校验在
    /// resolvedWord 内）。
    ...
    private static func resolvedWord(_ word: VocabularyWord, style: PanelStyle) -> String {
        ...
        let nativeValue = String(localized: String.LocalizationValue(nativeKey))
    ...
    func word(_ w: VocabularyWord) -> String {
...
/// demo 提取色值唯一落点（执行门 G2：Color 字面量仅 Theme.swift）。
