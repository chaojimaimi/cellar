...
> 状态：**0.3.0-alpha-dev**——一次性动作「充满一次」（充电到 100% 后自动恢复限充）已落地；核心限充与菜单栏图形界面随 0.2.0-alpha 发布，在 macOS 26 / Apple Silicon 真机完成端到端验收（安装 → 限充 → 放电恢复 → 睡眠唤醒 → 卸载）。欢迎试用与反馈，接口与行为可能调整。
