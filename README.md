...
> 状态：**0.3.1-alpha**——Phase 3 完成：界面风格系统（原生 / 酒窖琥珀）、「充满一次」、「放电到上限」、功率流向图、电池健康度、中英双语、`doctor` 扩展至十一项诊断。核心限充与菜单栏图形界面随 0.2.0-alpha 发布，在 macOS 26 / Apple Silicon 真机完成端到端验收（安装 → 限充 → 放电恢复 → 睡眠唤醒 → 卸载）。欢迎试用与反馈，接口与行为可能调整。

## 功能

- **任意上限限充**（60–100%）：含系统原生不支持的低于 80% 区间
- **滞回保持**：充到上限即停，自放电至恢复阈值（默认上限 −2%）才重新充电，避免频繁启停
- **充满一次**：临时充电到 100%（电池校准 / 出行前满电），完成后自动恢复限充
...
- **设置窗口**：外观切换、开机启动、登录项修复、通知入口
- **首启四步引导**：欢迎 → 环境检查（冲突门）→ 守护进程安装授权 → 设定上限
- **通知**：上限到达、写入失败、外部写者冲突、动作完成 / 安全终止等提醒（动作类豁免冷却）
- **开机自启登录项**：随登录自动常驻菜单栏
...
swift run CellarCoreCheck   # 149+ 个场景、数百项检查：决策矩阵穷举（700+ 边界组合）、
                            ...
                            # 放电安全门控、本地化完整性
swift run CellarUICheck     # 60 张界面快照对比 + 本地化完整性门
```
...
- Phase 4+：统计面板、校准模式、热保护、充电日程、Shortcuts 集成
