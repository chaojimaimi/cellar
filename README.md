...
> 状态：**0.16.0-alpha**（维护批）——Phase 5 全量能力在列：任意上限限充（60–100%）与滞回保持、充电侧温度暂停（阈值可配置）、风扇智能降温、可选自动放电、一键电池校准与自动校准调度、充电日程、「充满一次」/「放电到上限」、原生限充协调（macOS 26.4+）、MagSafe 指示灯控制，以及三套界面风格、统计仪表板、中英双语。本批为体验维护：菜单栏电量百分比开关、MagSafe LED 切换闪灰修复、引导页守卫文案与通用页弱化语义对齐。核心限充与菜单栏图形界面已在 macOS 26 / Apple Silicon 真机完成端到端验收（安装 → 限充 → 放电恢复 → 睡眠唤醒 → 卸载）。欢迎试用与反馈，接口与行为可能调整。
...
```bash
swift run CellarCoreCheck   # 539 个场景、数百项检查：决策矩阵穷举（700+ 边界组合）、
                            # 封包/解析、XPC 校验、策略持久化、动作状态机、通知分类、
                            # 放电安全门控、校准调度、热保护配置、原生限充检测、MagSafe LED、本地化完整性
bash Tools/coverage.sh      # 状态机行覆盖率门禁（圈定 Control/Daemon 纯逻辑，≥80%·当前 90.35%）
swift run CellarUICheck     # 294 张界面快照对比（三风格矩阵）+ 本地化完整性门
```
