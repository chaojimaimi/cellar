...
> 状态：**0.4.0-alpha**——电池保护补齐：充电侧温度暂停（≥ 40 °C 暂停 / < 37 °C 恢复）、可选自动放电（高于上限自动放回，默认关闭）、一键电池校准（四相满充-深放循环）、插拔即时执法（插电恢复充电 1-2 秒）。Phase 3 全部能力（界面风格系统、「充满一次」、「放电到上限」、功率流向图、电池健康度、中英双语、`doctor` 十一项诊断）随 0.3.1-alpha 发布，核心限充与菜单栏图形界面在 macOS 26 / Apple Silicon 真机完成端到端验收（安装 → 限充 → 放电恢复 → 睡眠唤醒 → 卸载）。欢迎试用与反馈，接口与行为可能调整。
