...
## [0.19.20-alpha] - 2026-09-16

### Added

- **限充执法恢复（macOS 27 编排通道）**：macOS 27 移除了 SMC 充电控制键族后，本版通过系统快捷指令通道恢复限充执法——守护进程负责决策（充电策略与日程 → 目标上限 → 行为验证重申），App 侧执行 `shortcuts run`。**一次性准备**：在快捷指令 App 新建指令，添加动作「设定电池充电上限」、参数选「输入快捷指令的信息」、命名为「设定电池充电上限」；然后在 Cellar 通用页打开「原生限充编排」开关即可。
- **充电日程恢复（macOS 27）**：日程命中改写目标上限并经编排通道执法；「放开充电」类条目在 27 上以「目标 100%」等价表达。
- **通用页编排节**：开关、快捷指令名自定义、执行状态与错误展示；doctor 新增「编排通道」检查项。
- 充电滑杆/日程在 27 上低于 80% 的目标按原生最低 80% 编排并如实标注（原生系统硬限 80–100%）。

### Changed

- macOS 27 上「充满一次」暂不可用（平台无执法通道，充满语义请将编排目标设为 100%）；原生限充检测在 27 上如实显示为「注册残留」而非现行上限。
- 守护进程能力清单在 27 上如实上报编排能力（此前为空清单）。

### Fixed

- 编排目标值变更不受冷却限制即时生效；同一目标的重申带 10 分钟冷却与悬挂超时（TTL），App 退出期间的请求不会重复执行（token 幂等）。
...
- **通用页风扇节「跳回默认值」显示陈旧（播种回灌）**：风扇滑杆/温度源 Picker 的值仅在视图首次创建时从守护进程状态播种一次——App 冷启动后状态到达前打开通用页，控件会持续显示出厂默认（电池温度源/37 °C），直到切页重建。现 daemon 状态首次到达时全量回灌（仅到达沿触发，不干扰在拖值）。该显示陈旧正是历史升级中「重装后设置变回默认」体感的成因（陈旧期交互会把陈旧值写回）。
- **调整任一滑杆时整节控件闪烁（细粒度 pending）**：原实现任一风扇参数提交都会禁用整节（含勾选框），回包后恢复——视觉上「闪一下」。现仅被提交的控件在自身往返期间短暂禁用，其余控件（含勾选框）全程可用；配套 fan 专用提交通道：飞行中的新提交进入单槽队列（latest-wins）在回包后自动补发，杜绝并发提交被静默丢弃。跨节副作用一并消除：风扇节不再随其他节操作（校准/放电等）的忙碌态禁用。

### 备注

- 已知边界：飞行窗口（数十至数百毫秒）内对另一控件的连续两笔提交取最后意图（latest-wins）；重试路径（横幅重试钮）与排队通道共用同一入口语义。
- daemon/充电控制路径零改动；golden 快照 330→336（新增 pending 形态 6 张，既有 330 张字节级零扰动）；场景 600/l10n 439/coverage 90.76% 不变。
