...
> Status: **0.19.7-alpha** (maintenance batch) — all Phase 5 capabilities in place: any-limit charge capping (60–100%) with hysteresis hold, charge-side thermal pause (configurable thresholds), smart fan cooling (synchronized left/right takeover on dual-fan machines), optional auto-discharge, one-click battery calibration with scheduled calibration, charge schedules, “Charge Once to Full” / “Discharge to Limit”, native charge-limit coordination (macOS 26.4+), and MagSafe LED control — plus three UI themes, a statistics dashboard, the menu-bar battery glyph (fill level / percentage / low-battery red), and zh/en bilingual UI. This batch polishes the General-page fan section: controls are re-seeded as soon as the daemon status arrives (fixing the stale “back to 37 °C battery source” display after cold launches — the root cause of settings appearing to reset after reinstalls), parameter commits switch to fine-grained pending (only the touched control dims briefly; the whole section no longer flickers), and commits issued mid-flight are queued in a single slot and re-sent automatically so no value is dropped. Core charge limiting and the menu-bar GUI completed end-to-end acceptance on real hardware (macOS 26 / Apple Silicon): install → limit → discharge recovery → sleep/wake → uninstall. Feedback and trial are welcome; interfaces and behavior may change.
...
- ✅ **Maintenance batch (0.19.6-alpha)**: auto-discharge replug-gate re-arm — after schedule-window or slider limit decreases, auto-discharge takes over automatically (an intentional limit decrease now clears the “replug adapter required” re-trigger gate); the battery no longer floats at the old limit (released)
