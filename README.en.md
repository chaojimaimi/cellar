...
> Status: **0.19.8-alpha** (maintenance batch) — all Phase 5 capabilities in place: any-limit charge capping (60–100%) with hysteresis hold, charge-side thermal pause (configurable thresholds), smart fan cooling (synchronized left/right takeover on dual-fan machines), optional auto-discharge, one-click battery calibration with scheduled calibration, charge schedules, “Charge Once to Full” / “Discharge to Limit”, native charge-limit coordination (macOS 26.4+), and MagSafe LED control — plus three UI themes, a statistics dashboard, the menu-bar battery glyph (fill level / percentage / low-battery red), and zh/en bilingual UI. This batch restores monitoring on macOS 27: the battery temperature source moves to SMC sensors (macOS 27 removed the ioreg Temperature key, which made the whole telemetry snapshot fail), doctor now states honestly that charge enforcement is unavailable on macOS 27 (the SMC control keys were removed; monitoring is unaffected), and CFBundleVersion now increments with every release. Core charge limiting and the menu-bar GUI completed end-to-end acceptance on real hardware (macOS 26 / Apple Silicon): install → limit → discharge recovery → sleep/wake → uninstall. Feedback and trial are welcome; interfaces and behavior may change.
...
```bash
swift run CellarCoreCheck   # 607 scenarios, hundreds of checks: exhaustive decision-matrix
